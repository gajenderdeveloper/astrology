...
class ZerodhaAPI:
    ...
    def get_all_strikes_with_ltp_and_oi(self,symbol, exchange="NFO", expiry_date="2025-06-26"):
        ...
        try:
            ...
            pivot_df = filtered_df.pivot(
                ...
                values=['day_low','day_high','ltp', 'oi', 'prev_oi','change_in_oi','volume','tradingsymbol','instrument_token','lot_size','name']
            )
    ...
    def pre_market_stock(self):
        """
        Fetch pre-market stock data for a specific symbol.
        This function is a placeholder and should be implemented based on your requirements.
        """
        # Get Nifty 50 stocks (example for pre-market movers)
        try:
            # Example: Fetch pre-market data for a specific symbol
            nifty_50_symbols = [
                'RELIANCE', 'HDFCBANK', 'ICICIBANK', 'INFY', 'HINDUNILVR',
                'TCS', 'KOTAKBANK', 'HDFC', 'ITC', 'SBIN', 'BAJFINANCE', 'BHARTIARTL',
                'AXISBANK', 'LT', 'MARUTI', 'ASIANPAINT', 'HCLTECH', 'TITAN', 'M&M',
                'ULTRACEMCO', 'SUNPHARMA', 'TECHM', 'TATAMOTORS', 'NTPC', 'ONGC',
                'INDUSINDBK', 'POWERGRID', 'NESTLEIND', 'BAJAJFINSV', 'JSWSTEEL',
                'ADANIPORTS', 'WIPRO', 'GRASIM', 'SBILIFE', 'HINDALCO', 'DRREDDY',
                'HDFCLIFE', 'DIVISLAB', 'BPCL', 'TATASTEEL', 'BRITANNIA', 'COALINDIA',
                'EICHERMOT', 'UPL', 'HEROMOTOCO', 'CIPLA', 'SHREECEM', 'IOC', 'GAIL',
                'VEDANTA'
            ]
            # Get quote for Nifty 50 stocks
            symbols = [f"NSE:{symbol}" for symbol in nifty_50_symbols]
            ...
            for symbol, data in quotes.items():
                pre_market_data.append({
                    ...
                    #'change_percentage': data['net_change_percent'],
                    'open': data['ohlc']['open'],
                    'high': data['ohlc']['high'],
                    'low': data['ohlc']['low'],
            ...
            # Sort by percentage change (pre-market movers)
            df = pd.DataFrame(pre_market_data)
            df['change'] = (df['last_price'] - df['close']).round(2)
            df['change_percentage'] = ((df['change'] / df['close']) * 100).round(2)
            pre_market_data = df.sort_values(by='change_percentage', ascending=False)

            
            return pre_market_data  # Return top 10 pre-market movers

        except Exception as e:
            logger.error(f"Error fetching pre-market stock data: {str(e)}")
            raise
